...
def get_family(ontology_edge_index, max_dist):
    ...
    df['family'] = df.progress_apply(lambda row: set.union(*[row[f'{dist}-neighbors'] for dist in range(1, max_dist+1)]), axis = 1)

    df['family_without_chidren'] = df.progress_apply(lambda row: row['family'] -set.union(*[row[f'{dist}-children'] for dist in range(1, max_dist+1)]), axis = 1)
    ...
    df = df[['source'] + [f'{dist}-neighbors' for dist in dists] + [f'{dist}-children' for dist in dists] + [f'{dist}-parents' for dist in dists]  + ['lineage', 'family']]
